...
**Measured in the simulation on 2026-09-07 over 6,000 campaigns**, in six disjoint blocks of 1,000 seeds, at difficulty tier 1, the `No tree` preset, the `triage` policy, static surges every 120 days for 5 dungeons, resolve floor ratio 2.0, escalation 0.10 per 100 days, craft 12 days for +4% of a tier width, with the Siege's city damage live and the play strategies able to see one: **the Last Stand is reached in 45.9% of campaigns and cleared 1 time in 24.6 of those reached** — 112 of 2,754 — against a **40.4%** win rate for the Cataclysm boss dungeon the player opened by clearing quest objectives, which itself opens in **54.9%** of campaigns. A campaign is won at all 24.1% of the time. **Every simulation figure in this document carries those conditions**, because a figure taken at another tier, policy or surge size is not comparable with one taken here.

**Those figures describe a player with NO empire tree.** The project owner ruled on 2026-09-06 that the game should be balanced around a player fully invested in the Explorer tree; nothing on record measures that player at these settings, so these numbers are not it.

**How firmly each of those is resolved, and one of them is still the least firm.** Across the six blocks the standard deviation is 1.27 points on the earned share, 2.19 on its win rate, 1.22 on the Last Stand share and 0.16 of a city on cities lost — and **0.34 points on the Last Stand clear rate, which is a twelfth of the figure itself**. The six blocks run 1 in 22 to 1 in 28, and the pooled 95% interval is 1 in 21 to 1 in 30. Read the clear rate as "about one in twenty-five" and nothing narrower.

**The clear rate is still the softest of the five, and by far less than it was.** As a standard deviation over its own figure it is 8.4%, against 5.4% for the next softest, the earned dungeon's win rate. On the 2026-09-06 run that this paragraph described until 2026-09-12 it was 31.1% against 4.2% — seven times its nearest rival where it is now one and a half times. **It is still the figure to be most careful with, and it is no longer in a class of its own.**

**The gap between two blocks is not a noise floor**: it is one realised difference rather than an estimate of a spread. **That comparison has not been re-made on the current model.** When it last was, on the superseded 2026-09-06 run, a two-block gap came in at a quarter of what six blocks showed; no figure for it is carried here, because none has been measured since.
...
**The owner's ruling that a collapse should be near-fatal is unaffected**, and has survived every re-measurement below. The fight is met in a little under half of campaigns and cleared in about one in twenty-five of them.
...
| **2026-09-07, today, on `a71ee1f`, six blocks of 1,000** | **45.9%** | **1 in 24.6** | **54.9%** | **40.4%** |
...
**A rate is per percent of maximum health rather than per point of it.** A character with 500 health and one with 5000 both gain 10 Fervour from losing a tenth of themselves. Per point, more health would mean a slower bar, which is the opposite of what the class is for. Two of the six generators are built in code, the Masochist's and the Ritualist's; `docs/DECISIONS.md`, 2026-08-25 and 2026-09-09, records the shape and what the rest would need. The Ravager's is designed and not built, and so are the Bulwark's, the Berserker's and the Saboteur's, which is why the table below has six rows and the code has two. The Ravager's is the largest of the four left: it is the only one whose row states a decay rule, and nothing in the game decays Fervour yet.
...
**A minion's blow provokes it, and the minion takes it.** Retaliation is paid back to whoever dealt the blow, and a minion deals its own blow rather than dealing it in its summoner's name, so the creature that swung takes what comes back and a summoner standing well away from the fight takes nothing. That a minion takes neither its summoner's critical strikes, penetration, weapon sub-type nor leech is the same rule read the other way: those four do not reach a minion's blow, and what a minion's blow provokes does not reach the summoner.
...
**"Multiplicative" and "more" are the same word.** Ten passive tree nodes grant damage reduction and say "(multiplicative)": nine in the Bulwark tree and one in the Saboteur tree. They mean "more", in the sense Path of Exile and Last Epoch both use it. The offensive pipeline above already uses "more" for exactly this, so the two sides of the game describe one idea.
...
**Every other loadout is measured against that one, and being above or below it is not an error.** A two-handed weapon deals about 1.32 times the target, which is the two-handed multiplier working rather than a loadout breaking the target. **That 1.32 is not the 1.29 the two-handed section below states, and the two do not disagree**: this figure is the composed damage per hit divided by the 1,860 target, and that one is the ratio of the weapon brackets alone, which `sim/analyse_two_handed_multiplier.py` solves. They are different quantities and both are checked against their own source by `tools/tests/test_two_handed_advantage_matches_the_design.py`. A single one-handed weapon deals about 0.81 times it, and a one-handed weapon with a Shield deals the same 0.81 since a Shield adds no attack damage; both trade damage for a free hand or for block and armor.
...
says, so a Quest dungeon in play actually moves on roughly **38% of its timers**
rather than 50%. Roughly one move in every two and a half timers: visible
without being constant.

  

The share with somewhere to go was **75.2% to 77.5%** and the share of timers
that moved the dungeon **37.6% to 38.7%**, measured over four disjoint blocks of
1,000 simulated campaigns — 115,064 quest timers — at difficulty tier 1 with no
empire tree, the triage policy, and static surges of five dungeons every 120
days. Those are block ranges rather than an error bar: four blocks give a
spread, and two would give only a difference.

  

**The figure recorded when the chance was ruled was 37%, and it was measured on
a different game.** That run predates both tying the active Cataclysm count to
the difficulty tier and opening the Cataclysm dungeon at half of them, and each
changes how long a campaign lasts and therefore how much of it is spent in
territory a Quest dungeon can still move through. The chance itself did not
change: take-up over the timers that had a choice measures 49.8%, which is the
coin.
