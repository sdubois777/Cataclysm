...
/**
 * An infested vein names no archetype row and is not mapped to one either: the save system skips every
 * floor source, so the empty name stays the sandbox training dummy's. Issues #1820 and #41.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FCataclysmSaveVeinClaimsNothing,
	"Cataclysm.SaveApply.AVeinDoesNotTakeTheTrainingDummysEmptyName",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCataclysmSaveVeinClaimsNothing::RunTest(const FString&)
{
...
/**
 * A Quarantine Breach containment names no archetype row and is not mapped to one either: the save system skips
 * every floor source, so the empty name stays the sandbox training dummy's. Issues #1820 and #41.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FCataclysmSaveQuarantineClaimsNothing,
	"Cataclysm.SaveApply.AQuarantineDoesNotTakeTheTrainingDummysEmptyName",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FCataclysmSaveQuarantineClaimsNothing::RunTest(const FString&)
{
	const ACataclysmQuarantineCharacter* Default = GetDefault<ACataclysmQuarantineCharacter>();
	if (!TestNotNull(TEXT("a containment class"), Default))
	{
		return false;
	}
	TestTrue(TEXT("it names no archetype row, as the dummy names none"), Default->ArchetypeRow.IsNone());
