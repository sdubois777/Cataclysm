	...
	UDataTable* RangedEffects(FAutomationTestBase& Test)
	{
		...
		const FString Csv =
			FString(TEXT("Name,Enchantment,Stat,ValueKind,ValueLow,ValueHigh,"
						 ...
						 "Action,ActionEvent,FractionOf,ScaleMaxSteps\n"))
			+ FString::Printf(TEXT("%s#1,%s,block_chance,increased,10,20,,,0,,0,,,,0\n"),
							  BlockBenefit, BlockBenefit)
			+ FString::Printf(TEXT("%s#1,%s,attack_speed,increased,-20,-35,,,0,,0,,,,0\n"),
