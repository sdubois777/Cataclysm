...
void ACataclysmHUD::DrawHUD()
{
	...
	// LAST, SO THE PANEL COVERS THE REST. A screen the player opened is what
	// they are looking at, and a damage number drawn over a grid cell would
	// read as part of the cell.
	if (bInventoryOpen)
	{
		DrawInventory();
	}
...
void ACataclysmHUD::ToggleInventory()
{
	bInventoryOpen = !bInventoryOpen;
}

bool ACataclysmHUD::InventoryCoversPoint(const FVector2D& Point) const
{
	if (!bInventoryOpen)
	{
		return false;
	...
	// THE PLAYER CONTROLLER ANSWERS THIS AND AHUD DOES NOT. AHUD's own Canvas is
	// only valid inside DrawHUD, and this is asked from an input handler.
	...
	if (SizeX <= 0 || SizeY <= 0)
	{
		// NO VIEWPORT MEANS NO PANEL TO HAVE BEEN CLICKED. Answering false makes
		// the click an ordinary move order, which is what it was before this
		// screen existed.
		return false;
	}

	return UCataclysmInventoryScreen::PanelCoversPoint(
		UCataclysmInventoryScreen::PanelRectFor(static_cast<float>(SizeX),
...
void ACataclysmHUD::DrawInventory()
{
	const APawn* Pawn = GetOwningPawn();
	if (!Pawn)
	{
	...
	const UCataclysmInventoryComponent* Inventory =
		Pawn->FindComponentByClass<UCataclysmInventoryComponent>();
	if (!Inventory)
	{
		// ONLY THE PLAYER CHARACTER CARRIES ANYTHING. A pawn possessed for some
		// other reason has no bag, so there is no grid to draw.
		return;
	}
	...
	const FBox2D Panel = UCataclysmInventoryScreen::PanelRectFor(Width, Height);
	const float CellPx = UCataclysmInventoryScreen::CellSizeFor(Width, Height);

	FLinearColor Backing = UCataclysmCombatOverlay::ColourFromHex(
		UCataclysmInventoryScreen::PanelHex);
	Backing.A = UCataclysmInventoryScreen::PanelOpacity;
	DrawRect(Backing, Panel.Min.X, Panel.Min.Y,
			 Panel.Max.X - Panel.Min.X, Panel.Max.Y - Panel.Min.Y);

	const FLinearColor Ink = UCataclysmCombatOverlay::ColourFromHex(
		UCataclysmInventoryScreen::HeaderTextHex);

	DrawTextCentred(
		UCataclysmInventoryScreen::HeaderTextFor(
			Inventory->NumItems(), UCataclysmInventoryComponent::SlotCount),
		Ink, static_cast<float>((Panel.Min.X + Panel.Max.X) * 0.5),
		static_cast<float>(Panel.Min.Y)
			+ UCataclysmInventoryScreen::PanelPaddingPx,
		UCataclysmInventoryScreen::HeaderScale);

	// THE FOUR TABLES ARE LOADED ONCE FOR THE WHOLE GRID. A cell fetching its
	// own would repeat these 48 times a frame for something that cannot change
	...
	const float LabelScale = UCataclysmInventoryScreen::LabelScaleFor(CellPx);

	const TArray<FCataclysmCarriedSlot>& Slots = Inventory->GetSlots();
	for (int32 Slot = 0; Slot < Slots.Num(); ++Slot)
	{
		const FBox2D Cell =
			UCataclysmInventoryScreen::CellRectFor(Panel, CellPx, Slot);
		if (!Cell.bIsValid)
		{
			continue;
		}

		const TArray<FString> Lines = UCataclysmInventoryScreen::LabelLinesFor(
			UCataclysmInventoryScreen::LabelFor(Slots[Slot], Bases, Materials),
			UCataclysmInventoryScreen::LabelCharactersPerLine,
			UCataclysmInventoryScreen::MaxLabelLines);

		DrawInventoryCell(
			Cell,
			UCataclysmInventoryScreen::ColourFor(Slots[Slot], Rarities,
												 Materials, Tiers),
			UCataclysmInventoryScreen::BorderThicknessFor(Slots[Slot],
														  Materials),
			Lines,
			UCataclysmInventoryScreen::QuantityTextFor(Slots[Slot]),
			LabelScale);
	}
}

void ACataclysmHUD::DrawInventoryCell(const FBox2D& Cell,
									  const FLinearColor& Frame,
									  int32 Thickness,
									  const TArray<FString>& Lines,
									  const FString& Quantity, float LabelScale)
{
	// THE FRAME FIRST, so the label sits on top of it rather than under it. The
	// same order DrawDropNames uses, and for the same reason.
	DrawBorder(Cell, static_cast<float>(Thickness), Frame);

	const float Inset = UCataclysmInventoryScreen::LabelInsetPx
		+ static_cast<float>(FMath::Max(0, Thickness));
	const float Inner =
		static_cast<float>(Cell.Max.X - Cell.Min.X) - Inset * 2.0f;
	if (Inner <= 0.0f)
	{
		return;
	}

	const float CentreX = static_cast<float>((Cell.Min.X + Cell.Max.X) * 0.5);

	// SHRUNK RATHER THAN CLIPPED WHEN A LINE IS STILL TOO WIDE. The wrapping
	// counted characters and characters are not all the same width, so a line
	// of wide ones can overrun a cell the count said it would fit. The canvas
	// cannot clip, so the choice is between a smaller label and one that spills
	// over the frame, and a smaller label is still the whole word.
	float Widest = 0.0f;
	for (const FString& Line : Lines)
	{
		const FBox2D Box = MeasureTextCentred(Line, 0.0f, 0.0f, LabelScale);
		Widest = FMath::Max(Widest, static_cast<float>(Box.Max.X - Box.Min.X));
	}

	float Scale = LabelScale;
	if (Widest > Inner)
	{
		Scale = LabelScale * Inner / Widest;
	}

	// MEASURED AGAIN AT THE FINAL SCALE, because the block is centred on the
	// cell and its height is not known until the scale is.
	TArray<FBox2D> Boxes;
	Boxes.Reserve(Lines.Num());
	float BlockHeight = 0.0f;
	for (const FString& Line : Lines)
	{
		const FBox2D Box = MeasureTextCentred(Line, CentreX, 0.0f, Scale);
		BlockHeight += static_cast<float>(Box.Max.Y - Box.Min.Y);
		Boxes.Add(Box);
	}

	float Top = static_cast<float>((Cell.Min.Y + Cell.Max.Y) * 0.5)
		- BlockHeight * 0.5f;
	for (int32 Index = 0; Index < Lines.Num(); ++Index)
	{
		DrawOutlinedText(Lines[Index], Frame,
						 static_cast<float>(Boxes[Index].Min.X), Top, Scale);
		Top += static_cast<float>(Boxes[Index].Max.Y - Boxes[Index].Min.Y);
	}
	...
	// THE COUNT IN THE BOTTOM RIGHT CORNER, IN THE TEXT COLOUR RATHER THAN THE
	// TIER'S. It is how many, not what: drawing it in the material's own colour
	// would make it read as another word of the name. Every game in the genre
	// puts a stack count in a corner.
	const FBox2D Counted = MeasureTextCentred(Quantity, 0.0f, 0.0f, Scale);
	DrawOutlinedText(
		Quantity,
		UCataclysmCombatOverlay::ColourFromHex(
			UCataclysmInventoryScreen::HeaderTextHex),
		static_cast<float>(Cell.Max.X) - Inset
			- static_cast<float>(Counted.Max.X - Counted.Min.X),
		static_cast<float>(Cell.Max.Y) - Inset
			- static_cast<float>(Counted.Max.Y - Counted.Min.Y),
		Scale);
}
