...
- **"A chance each time"**, answered with **"0.5"** once the curve was measured.
  ...
  each time, **and** the 37% of timers it works out to in play -- because a
...
class TestTheDocumentStatesTheMoveChance:
    ...
    **BOTH NUMBERS ARE CHECKED AND THAT IS THE POINT OF THE CLASS.** "A chance
    of 0.5" and "moves on 37% of its timers" are both true, because about a
    ...
    def test_it_states_the_consequence_in_timers_as_well(self, document):
        ...
        asked for it by name: "'a chance of 0.5' and 'moves 37% of the time' are
        both true and a reader will otherwise be surprised by one of them"."""
        assert "37% of its timers" in document, (
            ...
            "go regardless, so a Quest dungeon moves on roughly 37% of its "
    ...
    def test_it_states_the_conditions_the_figure_was_measured_at(self, document):
        ...
        assert "73.7% to 75.8%" in document, (
            "the design no longer states the measured share of quest timers "
            "that had somewhere adjacent to go, which is where the 37% comes "
            "from. Without it the 37% is an assertion rather than a result")
