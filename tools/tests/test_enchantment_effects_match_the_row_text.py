...
WHAT IS ASSERTED HERE.
    ...
    every row states one value, until the owner rules on ranges
    every value appears in its enchantment's own words, as a number or as the
      multiplying word the sentence uses ("Double", "tripled")
...
def test_every_row_states_one_value(effects):
    """How a range becomes one number on one item is open with the owner."""
    ranged = sorted(r["Name"] for r in effects
                    if float(r["ValueLow"]) != float(r["ValueHigh"]))
    assert not ranged, f"{ranged} state a range"


def test_every_value_appears_in_the_enchantments_own_words(effects,
                                                          enchantments):
    wrong = []
    for row in effects:
        ...
        said = abs(value) in numbers_in(text) or any(
            MULTIPLYING_WORDS.get(word) == value for word in words)
        if not said:
            wrong.append(f"{row['Name']}: {value:g} against {text!r}")
    assert not wrong, (
        "these values appear nowhere in their enchantment's words, as a number "
        "or as a multiplying word: " + "; ".join(wrong))
