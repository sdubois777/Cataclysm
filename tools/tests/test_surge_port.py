...
class TestWhatAQuestDungeonIs:
    ...
    def test_both_refuse_a_besieged_city_to_a_dungeon_carrying_a_siege(self):
        ...
        **THE COMMENTS ARE STRIPPED FIRST.** Both files explain the rule in a
        ...
        assert "bCarriesSiege" in picked and "SiegesPerCity" in picked, (
        ...
        assert "ECataclysmDungeonSubType::Siege" in moved, (
        ...
        assert "SiegesStanding" in moved, (
            "UCataclysmEmpireRun::RelocateQuestDungeon no longer counts the "
            "Sieges standing on the map. PickRelocation treats an empty array "
            "as \"the caller did not say\" and refuses nothing, so the check "
